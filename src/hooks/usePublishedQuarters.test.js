...
describe("resolveLandingQuarter", () => {
  ...
  it("waits while the lookup is still outstanding", () => {
    expect(resolveLandingQuarter({ quarter: current.suffix, quarterExplicit: false, published: null })).toBe(
      stay
    );
  });
  ...
  it("stays put when no quarter has anything", () => {
    expect(resolveLandingQuarter({ quarter: current.suffix, quarterExplicit: false, published: [] })).toBe(
      stay
    );
  });
