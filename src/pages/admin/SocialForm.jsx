...
import {
  parseLinkedInDemographics,
  AUDIENCE_DIMENSIONS,
  AUDIENCE_DIMENSION_LABELS,
  MAX_SEGMENTS,
          ...
          {rows.length > 0 && (
            <button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    `Clear all demographics for ${scopeLabel}? This cannot be undone unless you save first.`
                  )
                ) {
          ...
          return (
            <div key={dim.key} className="admin-demo-dim">
              <div className="admin-demo-dim-head">
                ...
                <span className="admin-demo-dim-count">
                  {dimRows.length} segment{dimRows.length === 1 ? "" : "s"}
                </span>
                <button
                  className="admin-btn-remove"
                  onClick={() => {
                    setDemographics((ds) =>
                      ds.filter((d) => !((d.campaign_id || null) === campaignId && d.dimension === dim.key))
                    );
                    dirty();
                  }}
                      ...
                      <tr key={row.id}>
                        <td>
                          <input
                            ...
                            onChange={(e) => {
                              setDemographics((ds) =>
                                ds.map((d) => (d.id === row.id ? { ...d, segment: e.target.value } : d))
                              );
                              dirty();
                            }}
                          />
                        ...
                        <td>
                          <input
                            type="number"
                            className="admin-input admin-input--cell r"
                            value={row.impressions}
                            onChange={(e) => {
                              setDemographics((ds) =>
                                ds.map((d) => (d.id === row.id ? { ...d, impressions: e.target.value } : d))
                              );
                              dirty();
                            }}
                          />
                        ...
                        <td>
                          <input
                            ...
                            onChange={(e) => {
                              setDemographics((ds) =>
                                ds.map((d) => (d.id === row.id ? { ...d, clicks: e.target.value } : d))
                              );
                              dirty();
                            }}
                          />
                        ...
                        <td>
                          <button
                            className="admin-btn-remove"
                            onClick={() => {
                              setDemographics((ds) => ds.filter((d) => d.id !== row.id));
                              dirty();
                            }}
    ...
    <div className="admin-demo-block">
          ...
          {rows.length > 0 && (
            <button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    `Clear all journeys for ${scopeLabel}? This cannot be undone unless you save first.`
                  )
                ) {
      ...
      <div className="admin-paste-row">
        ...
        <button
          ...
          disabled={!paste.trim()}
          onClick={() => {
            if (onImport(paste, campaignId)) setPaste("");
          }}
        >
          Add pasted rows
        </button>
        ...
        <>
          <div className="admin-posts-table-wrap">
            <table className="admin-posts-table admin-demo-table">
              <thead>
                <tr>
                  <th>Journey (pages in order, separated by &gt;)</th>
                  ...
                  <tr key={row.id}>
                    ...
                    <td>
                      <input
                        ...
                        className="admin-input admin-input--cell r"
                        value={row.sessions}
                        onChange={(e) => update(row.id, { sessions: e.target.value })}
                      />
                    </td>
                    <td>
                      <input
                        ...
                        onChange={(e) => update(row.id, { conversions: e.target.value })}
                      />
                    </td>
                    <td>
                      <button
                        className="admin-btn-remove"
                        onClick={() => {
                          setClickPaths((ps) => ps.filter((p) => p.id !== row.id));
                          dirty();
                        }}
      ...
      <button
        ...
        onClick={() => {
          setClickPaths((ps) => [...ps, newPathRow(campaignId)]);
          dirty();
        }}
      >
        + Add journey
...
function SaveBar({ saving, message, onSave }) {
  return (
    <div className="admin-save-bar">
      {message && (
        <span className={"admin-save-msg" + (message.startsWith("Error") ? " is-error" : "")}>{message}</span>
      )}
    ...
    (async () => {
      try {
        ...
        if (data) {
          ...
          setPlatforms(
            [...(data.social_platforms || [])]
              ...
              .map((p) => ({
                ...
                page_reach: str(p.page_reach),
                page_clicks: str(p.page_clicks),
                note: p.note || "",
              }))
          );
          ...
          setTopPosts(tp);
          setAllPosts(
            (data.social_posts || []).map((p) => ({
              post_name: p.post_name || "",
              post_date: p.post_date || "",
              post_time: p.post_time || "",
              platforms: p.platforms || "",
              impressions: str(p.impressions),
              engagements: str(p.engagements),
              url: p.url || "",
              notes: p.notes || "",
            }))
          );
          setDemographics(
            [...(data.paid_media_demographics || [])]
              ...
              .map((d) => ({
                ...
                is_other: !!d.is_other,
                impressions: str(d.impressions),
                clicks: str(d.clicks),
              }))
          );
          setClickPaths(
            [...(data.paid_media_click_paths || [])]
              .sort((a, b) => a.sort_order - b.sort_order)
              .map((p) => ({
                id: p.id,
          ...
          setInsights({
            ...
            actions: ins.actions || "",
            next_quarter: ins.next_quarter || "",
          });
          setCampaigns(
            [...(data.paid_media_campaigns || [])]
              .sort((a, b) => a.sort_order - b.sort_order)
              .map((c) => ({
                id: c.id,
                name: c.name || "",
                ...
                ads: [...(c.paid_media_ads || [])]
                  ...
                  .map((a) => ({
                    id: a.id,
                    name: a.name || "",
                    impressions: str(a.impressions),
                    reach: str(a.reach),
                    clicks: str(a.clicks),
                    cpc: str(a.cpc),
                    conversions: str(a.conversions),
                    engagement_rate: str(a.engagement_rate),
  ...
  return (
    <div className="admin-form">
            ...
            {KPI_FIELDS.map((f) => (
              <Field key={f.key} label={f.label}>
                <input
                  ...
                  onChange={(e) => {
                    setKpis((k) => ({ ...k, [f.key]: e.target.value }));
                    dirty();
                  }}
                />
              </Field>
      ...
      {tab === "platforms" && (
        <div className="admin-form-section">
          <div className="admin-list-hint">
            Add each social platform — LinkedIn, Facebook, Instagram, etc.
          </div>
          {platforms.map((p, i) => (
            <div key={i} className="admin-list-row">
              <div className="admin-list-row-grid admin-platform-grid">
                <Field label="Platform name">
                  <select
                    ...
                    value={p.name}
                    onChange={(e) => {
                      setPlatforms((ps) => ps.map((x, j) => (j === i ? { ...x, name: e.target.value } : x)));
                      dirty();
                    }}
                  >
                    <option value="">Select…</option>
                    {PLATFORMS_LIST.map((n) => (
                ...
                <Field label="Followers">
                  <input
                    ...
                    onChange={(e) => {
                      setPlatforms((ps) =>
                        ps.map((x, j) => (j === i ? { ...x, followers: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Engagement Rate (%)">
                  <input
                    ...
                    onChange={(e) => {
                      setPlatforms((ps) =>
                        ps.map((x, j) => (j === i ? { ...x, engagement_rate: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Page Reach">
                  <input
                    ...
                    onChange={(e) => {
                      setPlatforms((ps) =>
                        ps.map((x, j) => (j === i ? { ...x, page_reach: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Page Clicks">
                  <input
                    ...
                    onChange={(e) => {
                      setPlatforms((ps) =>
                        ps.map((x, j) => (j === i ? { ...x, page_clicks: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Note">
                  <input
                    ...
                    value={p.note}
                    onChange={(e) => {
                      setPlatforms((ps) => ps.map((x, j) => (j === i ? { ...x, note: e.target.value } : x)));
                      dirty();
                    }}
                  />
                </Field>
              ...
              <button
                className="admin-btn-remove"
                onClick={() => {
                  setPlatforms((ps) => ps.filter((_, j) => j !== i));
                  dirty();
                }}
          ...
          <button
            className="admin-btn-add"
            onClick={() => {
              setPlatforms((ps) => [...ps, { ...BLANK_PLATFORM }]);
              dirty();
            }}
          >
            + Add platform
      ...
      {tab === "allposts" && (
        <div className="admin-form-section">
          <div className="admin-allposts-toolbar">
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                ...
                disabled={allPosts.length < 2}
              >
                Sort by date {postsAsc ? "↑" : "↓"}
              </button>
              <button className="admin-btn-secondary" onClick={() => csvRef.current?.click()}>
                Import CSV
              </button>
              <button
                className="admin-btn-secondary"
          ...
          <p className="admin-csv-hint">
            CSV columns: <code>Post Name, Date, Time, Platforms, Impressions, Engagements, URL, Notes</code>{" "}
            (Time is optional)
          </p>
          <div className="admin-posts-table-wrap">
                  ...
                  <tr key={i}>
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, post_name: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, post_date: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, post_time: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, platforms: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, impressions: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, engagements: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, url: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => {
                          setAllPosts((ps) =>
                            ps.map((x, j) => (j === i ? { ...x, notes: e.target.value } : x))
                          );
                          dirty();
                        }}
                      />
                    ...
                    <td>
                      <button
                        className="admin-btn-remove"
                        onClick={() => {
                          setAllPosts((ps) => ps.filter((_, j) => j !== i));
                          dirty();
                        }}
                        aria-label="Remove"
                      >
          ...
          <button
            className="admin-btn-add"
            onClick={() => {
              setAllPosts((ps) => [...ps, { ...BLANK_ALL_POST, post_time: nowTimeStr() }]);
              dirty();
            }}
          >
            + Add row
      ...
      {tab === "paidmedia" && (
        <div className="admin-form-section">
          <div className="admin-list-hint">
            Group ads under a campaign, then add each ad with its metrics.
          </div>
          <datalist id="paid-platform-options">
            {PAID_PLATFORM_OPTIONS.map((p) => (
          ...
          {campaigns.map((c, ci) => (
            <div key={c.id} className="admin-campaign-card">
              <div className="admin-campaign-card-head">
                <Field label="Campaign name">
                  <input
                    ...
                    value={c.name}
                    onChange={(e) => {
                      setCampaigns((cs) => cs.map((x, j) => (j === ci ? { ...x, name: e.target.value } : x)));
                      dirty();
                    }}
                  />
                </Field>
              ...
              <div className="admin-campaign-meta">
                <Field label="Objective">
                  <input
                    ...
                    className="admin-input"
                    value={c.objective}
                    placeholder="Leads, Traffic, Awareness…"
                    onChange={(e) => {
                      setCampaigns((cs) =>
                        cs.map((x, j) => (j === ci ? { ...x, objective: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Platform">
                  <input
                    type="text"
                    className="admin-input"
                    list="paid-platform-options"
                    value={c.platform}
                    placeholder="LinkedIn, Facebook…"
                    onChange={(e) => {
                      setCampaigns((cs) =>
                        cs.map((x, j) => (j === ci ? { ...x, platform: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Budget ($)">
                  <input
                    ...
                    onChange={(e) => {
                      setCampaigns((cs) =>
                        cs.map((x, j) => (j === ci ? { ...x, budget: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Start date">
                  <input
                    ...
                    onChange={(e) => {
                      setCampaigns((cs) =>
                        cs.map((x, j) => (j === ci ? { ...x, start_date: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="End date">
                  <input
                    ...
                    onChange={(e) => {
                      setCampaigns((cs) =>
                        cs.map((x, j) => (j === ci ? { ...x, end_date: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
              ...
              {c.ads.map((a, ai) => (
                <div key={a.id} className="admin-list-row">
                          ...
                          setCampaigns((cs) =>
                            cs.map((x, j) =>
                              j === ci
                                ? {
                                    ...x,
                                    ads: x.ads.map((y, k) => (k === ai ? { ...y, name: e.target.value } : y)),
                                  }
                                : x
                            )
                          );
                            ...
                            cs.map((x, j) =>
                              j === ci
                                ? {
                                    ...x,
                                    ads: x.ads.map((y, k) =>
                                      k === ai ? { ...y, impressions: e.target.value } : y
                                    ),
                                  }
                                : x
                            )
                            ...
                            cs.map((x, j) =>
                              j === ci
                                ? {
                                    ...x,
                                    ads: x.ads.map((y, k) =>
                                      k === ai ? { ...y, clicks: e.target.value } : y
                                    ),
                                  }
                                : x
                            )
                          ...
                          setCampaigns((cs) =>
                            cs.map((x, j) =>
                              j === ci
                                ? {
                                    ...x,
                                    ads: x.ads.map((y, k) => (k === ai ? { ...y, cpc: e.target.value } : y)),
                                  }
                                : x
                            )
                          );
                            ...
                            cs.map((x, j) =>
                              j === ci
                                ? {
                                    ...x,
                                    ads: x.ads.map((y, k) =>
                                      k === ai ? { ...y, conversions: e.target.value } : y
                                    ),
                                  }
                                : x
                            )
                            ...
                            cs.map((x, j) =>
                              j === ci
                                ? {
                                    ...x,
                                    ads: x.ads.map((y, k) =>
                                      k === ai ? { ...y, engagement_rate: e.target.value } : y
                                    ),
                                  }
                                : x
                            )
                            ...
                            cs.map((x, j) =>
                              j === ci
                                ? {
                                    ...x,
                                    ads: x.ads.map((y, k) =>
                                      k === ai ? { ...y, status: e.target.value } : y
                                    ),
                                  }
                                : x
                            )
                  ...
                  <button
                    ...
                    onClick={() => {
                      setCampaigns((cs) =>
                        cs.map((x, j) => (j === ci ? { ...x, ads: x.ads.filter((_, k) => k !== ai) } : x))
                      );
                      dirty();
                    }}
              ...
              <button
                className="admin-btn-add"
                onClick={() => {
                  setCampaigns((cs) => cs.map((x, j) => (j === ci ? { ...x, ads: [...x.ads, newAd()] } : x)));
                  dirty();
                }}
              >
                + Add ad
          ...
          {[
            ...
            <Field key={s.key} label={s.label}>
              <textarea
                className="admin-textarea"
                rows={6}
                value={insights[s.key]}
                onChange={(e) => {
                  setInsights((ins) => ({ ...ins, [s.key]: e.target.value }));
                  dirty();
                }}
              />
            </Field>
