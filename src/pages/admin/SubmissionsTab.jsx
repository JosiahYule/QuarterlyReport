  ...
  return (
    <div className="admin-lead-block">
      ...
      <div className="admin-list-hint">
        Businesses looking for staff. The most recent {Math.min(leads.length, 6)} shown; these are the rows
        worth a same-day call back.
      </div>
...
export function SubmissionsTab({ agency }) {
  ...
  const importCsv = async (file) => {
    ...
    try {
      ...
      flash(
        `Imported ${parsed.length} rows — ${added} new, ${parsed.length - added} already present` +
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(
      (r) =>
        ...
        (!q ||
          [r.name, r.email, r.phone, r.comments, r.source_detail].some(
  ...
  useEffect(() => {
    setShown(PAGE);
  }, [search, fIntent, fLocation, fSource]);
    ...
    <div className="admin-form">
      <div className="admin-allposts-toolbar">
        <div className="admin-list-hint" style={{ marginBottom: 0 }}>
          Contact form submissions — {rows.length} on record. Import the form's CSV export; re-importing
          overlapping exports only adds what's new.
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {msg && (
            <span className={"admin-save-msg" + (msg.startsWith("Error") ? " is-error" : "")}>{msg}</span>
          )}
          <input
            ref={csvRef}
            type="file"
      ...
      <div className="admin-sub-filters" ref={tableRef}>
        <input
          className="admin-input admin-sub-search"
          type="search"
          placeholder="Search name, email, phone, comments…"
          value={search}
      ...
      <div className="admin-posts-table-wrap">
        <table className="admin-posts-table">
          ...
          <tbody>
            {filtered.slice(0, shown).map((r) => {
              ...
              return (
                <React.Fragment key={r.id}>
                  <tr
                    className={hasMore ? "admin-sub-row--expandable" : ""}
                  ...
                  {isOpen && (
                    <tr className="admin-sub-detail">
                      <td colSpan={8}>
                        {r.source_detail && (
                          <p>
                            <strong>Source detail:</strong> {r.source_detail}
                          </p>
                        )}
            ...
            {!filtered.length && (
              <tr>
                <td colSpan={8} className="admin-sub-empty">
