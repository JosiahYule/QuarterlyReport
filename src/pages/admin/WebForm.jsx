        ...
        if (data) {
          ...
          setChannels(
            [...(data.web_channels || [])]
              ...
              .map((c) => ({
                name: c.name,
                sessions: str(c.sessions),
                share_of_traffic: str(c.share_of_traffic),
                engagement_rate: str(c.engagement_rate),
              }))
          );
          setPages(
            [...(data.web_pages || [])]
              ...
              .map((p) => ({
                key: p.key,
                page_views: str(p.page_views),
                bounce_rate: str(p.bounce_rate),
        ...
        <div className="admin-form-section">
          <Field label="Executive summary / bullet">
            <textarea
              ...
              value={summaryBullet}
              onChange={(e) => {
                setSummaryBullet(e.target.value);
                dirty();
              }}
      ...
      {tab === "channels" && (
        <div className="admin-form-section">
          <div className="admin-list-hint">
            Traffic channels from GA4 — Organic Search, Direct, Social, etc.
          </div>
          {channels.map((c, i) => (
              ...
              <div className="admin-list-row-grid admin-channel-grid">
                ...
                <Field label="Sessions">
                  <input
                    ...
                    onChange={(e) => {
                      setChannels((cs) =>
                        cs.map((x, j) => (j === i ? { ...x, sessions: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Share of Traffic (%)">
                  <input
                    ...
                    step="0.001"
                    className="admin-input"
                    value={c.share_of_traffic}
                    onChange={(e) => {
                      setChannels((cs) =>
                        cs.map((x, j) => (j === i ? { ...x, share_of_traffic: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Engagement Rate (%)">
                  <input
                    ...
                    step="0.001"
                    className="admin-input"
                    value={c.engagement_rate}
                    onChange={(e) => {
                      setChannels((cs) =>
                        cs.map((x, j) => (j === i ? { ...x, engagement_rate: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
          ...
          <button
            ...
            onClick={() => {
              setChannels((cs) => [...cs, { ...BLANK_CHANNEL }]);
              dirty();
            }}
          >
            + Add channel
              ...
              <div className="admin-list-row-grid admin-page-grid">
                <Field label="Page / path">
                  <input
                    ...
                    className="admin-input"
                    value={p.key}
                    placeholder="/jobs, Homepage…"
                    onChange={(e) => {
                      setPages((ps) => ps.map((x, j) => (j === i ? { ...x, key: e.target.value } : x)));
                      dirty();
                    }}
                  />
                </Field>
                <Field label="Page Views">
                  <input
                    ...
                    onChange={(e) => {
                      setPages((ps) =>
                        ps.map((x, j) => (j === i ? { ...x, page_views: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Bounce Rate (%)">
                  <input
                    ...
                    onChange={(e) => {
                      setPages((ps) =>
                        ps.map((x, j) => (j === i ? { ...x, bounce_rate: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
                ...
                <Field label="Avg Time on Page (sec)">
                  <input
                    type="number"
                    className="admin-input"
                    value={p.avg_time_on_page_sec}
                    onChange={(e) => {
                      setPages((ps) =>
                        ps.map((x, j) => (j === i ? { ...x, avg_time_on_page_sec: e.target.value } : x))
                      );
                      dirty();
                    }}
                  />
