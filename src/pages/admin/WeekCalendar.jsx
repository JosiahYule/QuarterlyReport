...
export function WeekCalendar({ agency, quarter, posts, week, quarterStart, quarterEnd }) {
  ...
  const maxOffset = quarterEnd
    ? Math.round((mondayOf(addDays(quarterEnd, -1)) - thisMonday) / (7 * DAY_MS))
    : 52;
  ...
  return (
    <div>
      ...
      <p className="admin-list-hint">
        {openCount > 0
          ? `${openCount} open item${openCount === 1 ? "" : "s"} this quarter.`
          : "Nothing queued yet."}
      </p>
      ...
      <div className="admin-cal-nav">
        ...
        <span className="admin-cal-range">
          {fmtShort(weekDates[1])} – {fmtShort(weekDates[5])}
        </span>
        {weekOffset !== 0 && (
          <button className="admin-btn-ghost admin-cal-today" onClick={() => setWeekOffset(0)}>
            Today
          </button>
        )}
        <button
      ...
      {error && (
        <div className="admin-form-status admin-form-status--error" style={{ marginTop: 12 }}>
          {error}
        </div>
      )}
          ...
          {WEEKDAYS.map((dayIndex) => {
            ...
            return (
              <div
                key={dayIndex}
                className={
                  "admin-cal-day" + (isToday ? " is-today" : "") + (isEmpty && isPast ? " is-missed" : "")
                }
              >
                ...
                {dayItems.map((item) => (
                  <div
                    key={item.id}
                    className={"admin-cal-item" + (item.status === "posted" ? " is-posted" : "")}
                  >
                    <input
                  ...
                  (hasSuggestion ? (
                    <div className="admin-cal-suggestion">
                      <span className="admin-cal-ghost">
                        {suggestion.slot === "job"
                          ? `${suggestion.roleLabel} job ad`
                          : suggestion.bestType?.label || "Mix it up — any fresh content"}
                      </span>
                      <button
      ...
      <div className="admin-cal-backlog">
        ...
        <p className="admin-list-hint">
          Ideas without a date yet — schedule one to move it onto the calendar.
        </p>
        ...
        <div className="admin-planner-add">
          ...
          <button className="admin-btn-primary" onClick={() => add()} disabled={!draft.idea.trim()}>
            Add
          </button>
        </div>
        ...
        {backlog.length === 0 ? (
          <div className="admin-plan-empty">
            No backlog ideas. Add one above, or add directly to a day in the calendar.
          </div>
        ) : (
          <ul className="admin-planner-list">
            {backlog.map((item) => (
              <li key={item.id} className={"admin-planner-row is-" + item.status}>
                <select
                  className="admin-input admin-planner-status"
                  value={item.status}
                  onChange={(e) => patch(item.id, { status: e.target.value })}
                >
                  {STATUSES.map((s) => (
                ...
                />
                <select
                  className="admin-input admin-planner-type"
                  value={item.content_type || ""}
                  onChange={(e) => patch(item.id, { content_type: e.target.value || null })}
                >
                  <option value="">Type…</option>
                ...
                <input
                  type="date"
                  className="admin-input admin-planner-date"
                  value=""
                  onChange={(e) => e.target.value && patch(item.id, { planned_date: e.target.value })}
                />
                <button className="admin-btn-remove" onClick={() => remove(item.id)} aria-label="Remove item">
                  <IconClose />
                </button>
              </li>
            ))}
